...
async function loadManagerPayload(client: SupabaseClient, role: string) {
  ...
  const categories = (categoriesResult.data || []).filter((row) => row.is_active !== false && row.status !== "inactive");
  const categoryById = new Map(categories.map((row) => [String(row.id), row]));
  const variants = variantsResult.data || [];
  const sellable = sellableResult.data || [];
  const sizeMaster = sizeMasterResult.data || [];
  ...
  const products = await Promise.all((productsResult.data || []).map(async (row) => {
    const id = String(row.id);
    const variantIds = variantIdsByProduct.get(id) || [];
    const snapshot = await buildSnapshotFromLoaded(client, row, categoriesResult.data || [], variants, sellable, images, sizeMaster);
    return {
      id,
