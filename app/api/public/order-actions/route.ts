...
export async function POST(request: Request) {
  ...
  const respond = (
    ...
    body,
    status,
    observabilityResponseHeaders(observability, headers)
  );
  try {
    ...
    if (!orderNumber || !new Set(["request_cancellation", "request_pickup_extension"]).has(action)) {
      return respond({ error: "Tindakan pesanan tidak valid." }, 400);
    }

    const client = getAdminSupabaseClient();
    if (!client) return respond({ error: "Layanan pesanan belum tersedia." }, 503);
    ...
    if (!authorization.ok || !order) {
      return respond({ error: "Data pesanan tidak cocok atau sudah tidak tersedia." }, 404);
    }
    ...
    if (rateLimitError) {
      logServerError(observability, rateLimitError, {
        event: "public_order_action.rate_limit_unavailable"
      });
      return respond(
        {
          code: "ORDER_ACTION_UNAVAILABLE",
          error: "Perlindungan permintaan sedang tidak tersedia. Coba lagi nanti."
        },
        503,
        { "retry-after": "60" }
      );
    ...
    if (Number(count ?? 0) >= 5) {
      return respond({ error: "Terlalu banyak permintaan. Coba lagi nanti." }, 429, { "retry-after": "3600" });
    }
    ...
    if (action === "request_cancellation") {
      ...
      if (reason.length < 5 || reason.length > 1000) {
        return respond({ error: "Jelaskan alasan pembatalan minimal 5 karakter." }, 400);
      }
      ...
      if (reason.length < 5 || Number.isNaN(new Date(requestedDeadline).getTime())) {
        return respond({ error: "Alasan dan waktu perpanjangan wajib diisi." }, 400);
      }
    ...
    if (auditError) {
      logServerError(observability, auditError, {
        event: "public_order_action.audit_write_failed",
        entityType: "order",
        entityId: order.id,
        action
      });
    ...
    return respond({ ok: true, action, result }, 201);
  } catch (error) {
