...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return Response.json({ code: "CUSTOMER_RESEND_INVALID", error: "Email tidak valid." }, { status: 400 });
  const recaptcha = await verifyRecaptchaToken(
    typeof record.recaptchaToken === "string" ? record.recaptchaToken : null,
    "customer_resend_verification"
  );
  if (!recaptcha.valid) return Response.json({ code: "CUSTOMER_RESEND_SECURITY", error: recaptcha.message ?? "Verifikasi keamanan gagal." }, { status: recaptcha.configured ? 403 : 503 });
  ...
  if (internal !== true) {
    await auth.auth.resend({
      ...
      options: { emailRedirectTo: `${getSiteUrl()}/auth/callback?next=%2Faccount` }
    });
  }
  return Response.json({ accepted: true, message: "Jika akun dapat diverifikasi, email baru akan dikirim." }, { status: 202 });
}
