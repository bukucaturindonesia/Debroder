...
Mengatur field dimensions, labels, helper/error text, spacing, focus, validation, dan accessibility.

## Scope

Checkout, account, admin forms, search-like inputs, address, identity, dan other structured input.
...
- Input height, radius, padding, label/helper/error sizes mengikuti Master.
- Error tampil dekat field dan menjawab masalah secara jelas.
- Loading/disabled tidak boleh mengubah layout.
- Business validation rule tidak ditentukan oleh UI spec ini.
...
↓ 6–8px
Field
↓ helper/error bila ada
↓ 16–20px menuju field berikutnya
```
...
UI specification tidak membuat validation business rules. Ia hanya menentukan bahwa validation state:

- dekat dengan field terkait;
- menggunakan error typography/semantic red;
- mempunyai copy yang menjelaskan masalah;
- tidak hanya mengubah border color tanpa message jika pengguna membutuhkan explanation.

## Disabled / Loading

Disabled field tetap readable dan tidak kehilangan label. Loading state tidak mengubah field height.

## Accessibility

Label harus berasosiasi dengan field. Error harus dapat dikaitkan dengan field. Placeholder bukan pengganti label jika label diperlukan untuk memahami input.
