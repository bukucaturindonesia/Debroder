...
Menetapkan urutan resmi pekerjaan UI untuk mengurangi drift antar token, responsive, state, icon, feedback, dan QA.

## Scope

Semua implementation package UI DEBRODER.
...
- WARNA→DIMENSI KOMPONEN→RESPONSIVE→STATE→IKON→FEEDBACK→QA.
- Urutan tidak boleh direorder tanpa owner decision.
- Open decisions pada warna/icon/responsive/media tetap harus dihormati sebelum langkah terkait difinalkan.
