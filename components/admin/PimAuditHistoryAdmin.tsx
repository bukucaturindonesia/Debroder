  ...
  return (
    <dialog ref={ref} aria-labelledby="pim-audit-detail-title" onCancel={(event) => { event.preventDefault(); onClose(); }} className="m-0 ml-auto h-full max-h-none w-full max-w-3xl overflow-y-auto border-0 bg-white p-0 shadow-2xl backdrop:bg-black/40">
      ...
      <div className="space-y-7 p-5 sm:p-6">
        <section className="grid gap-3 sm:grid-cols-2"><Meta label="Event code" value={`${detail.eventCode} · v${detail.eventVersion}`} /><Meta label="Kategori" value={detail.category} /><Meta label="Status" value={STATUS_LABELS[detail.status] || detail.status} /><Meta label="Waktu" value={formatDate(detail.createdAt)} /><Meta label="Admin" value={`${detail.actorLabel} · ${getRoleLabel(detail.actorRole)}`} /><Meta label="Modul" value={detail.sourceModule} /><Meta label="Entity" value={`${detail.entityType}${detail.entityLabel ? ` · ${detail.entityLabel}` : ""}`} /><Meta label="SKU" value={detail.sku || "—"} /><Meta label="Batch ID" value={detail.batchId || "—"} mono /><Meta label="Request ID" value={detail.requestId || "—"} mono /><Meta label="Operation ID" value={detail.operationId || "—"} mono /><Meta label="Durasi" value={detail.durationMs === null ? "—" : `${detail.durationMs} ms`} /><Meta label="Failure code" value={detail.failureCode || "—"} /></section>
