...
export function WebsiteSettingsAdmin() {
  ...
  async function loadSettings() {
    ...
    setLoading(false);
    if (error) {
      setStatus("Pengaturan website belum dapat dimuat. Coba lagi.");
      return;
    }
  ...
  async function saveSetting(event: FormEvent) {
    ...
    if (result.error) {
      setStatus("Pengaturan belum dapat disimpan. Periksa data lalu coba lagi.");
      return;
    }
  ...
  async function deleteSetting(setting: WebsiteSetting) {
    ...
    setStatus(error ? "Pengaturan belum dapat dihapus. Coba lagi." : "Pengaturan dihapus.");
  ...
  return (
    <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(300px,.75fr)_minmax(0,1.25fr)]">
      <form onSubmit={saveSetting} className="bg-white p-5 sm:p-6">
        <div className="flex items-center justify-between gap-3"><h2 className="text-xl font-semibold">{editingId ? "Edit Pengaturan" : "Pengaturan Baru"}</h2>{editingId ? <button type="button" onClick={reset} className="text-sm font-semibold underline">Batal</button> : null}</div>
        {status ? <p role="status" className="mt-4 bg-brand-offWhite p-3 text-sm font-semibold">{status}</p> : null}
        <div className="mt-5 grid gap-4">
          <label className="grid gap-2 text-sm font-semibold">Kunci pengaturan<input required value={form.setting_key} onChange={(event) => setForm((current) => ({ ...current, setting_key: event.target.value }))} disabled={!!editingId} className="min-h-11 rounded-lg border border-brand-softGray px-4 font-normal disabled:bg-brand-offWhite" /></label>
          ...
          <label className="grid gap-2 text-sm font-semibold">Nilai pengaturan (JSON)<textarea required rows={8} value={form.value} onChange={(event) => setForm((current) => ({ ...current, value: event.target.value }))} className="rounded-lg border border-brand-softGray p-4 font-mono text-xs font-normal" /></label>
          ...
          <button disabled={saving} className="min-h-11 rounded-full bg-brand-green px-6 text-sm font-semibold text-white disabled:opacity-50">{saving ? "Menyimpan..." : "Simpan Pengaturan"}</button>
        </div>
      </form>
      ...
      <section className="bg-white p-5 sm:p-6">
        ...
        ))}</div> : <p className="mt-5 bg-brand-offWhite p-5 text-sm text-brand-charcoal/60">Belum ada pengaturan website.</p>}
      </section>
    </div>
  );
}
