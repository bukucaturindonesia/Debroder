...
function resolveOperationalState(order: Props["order"], job: DomainRef, qc: Props["qualityControl"], fulfillment: DomainRef) {
  ...
  if (["preparing", "packing", "problem"].includes(fulfillment?.status ?? "")) return result(8, "Pengemasan", fulfillment?.status === "problem" ? "Pengiriman bermasalah" : "Tidak ada", "Lanjutkan Pengiriman", `/admin/fulfillments/${fulfillment?.id}`, `/admin/fulfillments/${fulfillment?.id}`);
