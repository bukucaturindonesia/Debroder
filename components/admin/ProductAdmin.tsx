...
export function ProductAdminPanel() {
  ...
  const canEditCurrent = !selected || currentStatus === "draft" || Boolean(payload?.capabilities.canEditPublished);
      ...
      <AdminPageHeader
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={newDraft} className="rounded-full bg-brand-charcoal px-5 py-3 text-sm font-semibold text-white">
              Produk Draft Baru
            </button>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.86fr_1.14fr]">
        <form onSubmit={save} className="self-start bg-white p-5 sm:p-7 xl:sticky xl:top-24">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <button disabled={!canEditCurrent || working} className="min-h-12 rounded-full bg-brand-green px-6 text-sm font-semibold text-white disabled:opacity-45">
              ...
              <article key={product.id} className="border border-brand-softGray p-4 sm:p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    <button type="button" onClick={() => applyProduct(product)} className="rounded-full bg-brand-charcoal px-4 py-2 text-xs font-semibold text-white">Edit</button>
                    <button type="button" onClick={() => void runAction("duplicate", { productId: product.id })} disabled={working} className="rounded-full border border-brand-softGray px-4 py-2 text-xs font-semibold disabled:opacity-45">Duplikat Draft</button>
                    {payload?.capabilities.canPublish && product.status === "draft" ? <button type="button" onClick={() => void runAction("publish", { productId: product.id })} disabled={working || product.validationIssues.length > 0} className="rounded-full bg-brand-green px-4 py-2 text-xs font-semibold text-white disabled:opacity-40">Publish</button> : null}
                    {payload?.capabilities.canArchive && product.status === "active" ? <button type="button" onClick={() => void runAction("archive", { productId: product.id })} disabled={working} className="rounded-full border border-amber-300 px-4 py-2 text-xs font-semibold text-amber-800 disabled:opacity-45">Archive</button> : null}
                  </div>
                </div>
