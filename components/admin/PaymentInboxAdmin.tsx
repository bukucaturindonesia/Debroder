...
export function PaymentInboxAdmin() {
  ...
  return <main className="grid gap-6 text-brand-charcoal">
    <AdminPageHeader eyebrow="WORK QUEUE" title="Pembayaran" description="Periksa bukti pembayaran dari satu antrean, lalu lanjutkan keputusan pada workspace order canonical." />
    ...
    <section className="grid gap-4">
      {filtered.map((row) => { const order = Array.isArray(row.orders) ? row.orders[0] : row.orders; return <article key={row.id} className="grid gap-5 border border-brand-softGray bg-white p-5 lg:grid-cols-[96px_1fr_auto] lg:items-center">
        ...
        <div>
          ...
          <p className="mt-2 text-sm font-semibold">{order?.order_number ?? row.order_id} · {order?.customer_name ?? "Pelanggan"}</p>
          <div className="mt-3 grid gap-1 text-sm text-brand-charcoal/60 sm:grid-cols-2"><p>Dikirim: <strong>{money(row.amount)}</strong></p><p>Sisa order: <strong>{money(order?.payment_balance ?? 0)}</strong></p><p>Metode: {row.method.replaceAll("_", " ")}</p><p>Waktu: {dateTime(row.submitted_at ?? row.created_at)}</p></div>
...
function statusLabel(status: string) { return ({ pending: "Menunggu Verifikasi", verified: "Terverifikasi", rejected: "Ditolak", refunded: "Koreksi / Refund", draft: "Draft" } as Record<string, string>)[status] ?? status; }
