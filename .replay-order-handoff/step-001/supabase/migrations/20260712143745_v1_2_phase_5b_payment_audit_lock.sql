...
-- The historical submit_public_payment_proof(uuid,text,text,text) RPC is
-- intentionally not reconstructed in the fresh baseline.  Its old body
-- writes legacy orders.payment_proof_path/payment_status fields, accepts
-- order-number/phone matching as authorization, and is absent from the
-- CURRENT HEAD public payment flow.  Remove its legacy ACL when a preserved
-- historical database still has it, but keep fresh replay from requiring an
-- obsolete mutation API.
do $legacy_payment_proof_acl$
begin
