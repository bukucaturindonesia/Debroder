-- Wave 0 runtime correction: the global store-scope policies are restrictive
-- for authenticated staff, but customers do not have a public.profiles role.
-- NULL <> 'store_admin' is NULL, so the old predicate accidentally denied
-- customer rows even when a public/customer permissive policy allowed them.
-- Preserve store-admin scope while letting the applicable domain policy make
-- the customer/guest decision.
  ...
  for policy_row in
    ...
    from (values
      ('orders', 'global dashboard store scope orders',
        '(public.current_actor_role() is null or public.current_actor_role() <> ''store_admin'' or public.can_access_order(id))'),
      ('order_items', 'global dashboard store scope order items',
        '(public.current_actor_role() is null or public.current_actor_role() <> ''store_admin'' or public.can_access_order(order_id))'),
      ...
      ('inventory_locations', 'global dashboard store scope inventory locations',
        '(public.current_actor_role() is null or public.current_actor_role() <> ''store_admin'' or public.can_access_store(store_id))'),
      ('inventory_balances', 'global dashboard store scope inventory balances',
        '(public.current_actor_role() is null or public.current_actor_role() <> ''store_admin'' or public.can_access_inventory_location(location_id))'),
      ('stores', 'global dashboard store scope stores',
        '(public.current_actor_role() is null or public.current_actor_role() <> ''store_admin'' or public.can_access_store(id))')
...
begin
  if to_regclass('public.work_items') is not null then
    ...
    create policy "global dashboard store scope work items"
      ...
      using (
        public.current_actor_role() is null
        or public.current_actor_role() <> 'store_admin'
        or exists (
          select 1
