-- These signatures belong to the retired Phase 11 draft/tracking contract.
-- Fresh CURRENT HEAD installs do not create them, while legacy upgrade
-- environments may still contain them.  Revoke them only when present so a
-- fresh replay remains executable without restoring the retired API.
do $$
begin
  if to_regprocedure('public.create_fulfillment(uuid,text,text,text,text,text,integer,timestamptz,text,jsonb)') is not null then
    execute 'revoke all on function public.create_fulfillment(uuid,text,text,text,text,text,integer,timestamptz,text,jsonb) from public,anon,authenticated';
  end if;
  if to_regprocedure('public.update_fulfillment_draft(uuid,text,text,text,text,integer,timestamptz,text)') is not null then
    execute 'revoke all on function public.update_fulfillment_draft(uuid,text,text,text,text,integer,timestamptz,text) from public,anon,authenticated';
  end if;
  if to_regprocedure('public.update_fulfillment_tracking(uuid,text,text,text)') is not null then
    execute 'revoke all on function public.update_fulfillment_tracking(uuid,text,text,text) from public,anon,authenticated';
  end if;
  if to_regprocedure('public.transition_fulfillment_status(uuid,text,text)') is not null then
    execute 'revoke all on function public.transition_fulfillment_status(uuid,text,text) from public,anon,authenticated';
  end if;
  if to_regprocedure('public.refresh_order_fulfillment_status(uuid)') is not null then
    execute 'revoke all on function public.refresh_order_fulfillment_status(uuid) from public,anon,authenticated';
  end if;
end
$$;
