...
begin
  if new.pricing_mode in ('configurator_based','custom_quote') or new.uses_configurator then
    if new.sales_mode <> 'both' then
      new.sales_mode := 'custom';
    end if;
    new.tier_scope := 'none';
...
revoke all on function public.normalize_product_commerce_defaults_v1() from public, anon, authenticated;
grant execute on function public.normalize_product_commerce_defaults_v1() to service_role;

drop trigger if exists trg_normalize_product_commerce_defaults_v1 on public.products;
...
on public.products
for each row
execute function public.normalize_product_commerce_defaults_v1();
...
begin
  ...
  if affected_product_id is not null then
    ...
    set tier_scope = case
      when p.pricing_mode in ('configurator_based','custom_quote') or p.uses_configurator then 'none'
      when exists (
        select 1
...
revoke all on function public.sync_product_tier_scope_v1() from public, anon, authenticated;
grant execute on function public.sync_product_tier_scope_v1() to service_role;

drop trigger if exists trg_sync_product_tier_scope_v1 on public.product_price_tiers;
...
for each row
execute function public.sync_product_tier_scope_v1();
