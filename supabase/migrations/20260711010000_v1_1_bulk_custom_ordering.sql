...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'product_price_tiers_no_overlap_active'
  ) then
...
alter table public.custom_services
  ...
  add column if not exists estimated_min_price integer,
  add column if not exists estimated_max_price integer,
  ...
  add column if not exists allowed_file_types text[] not null default array['png','jpg','jpeg','pdf','svg','ai','eps','zip'],
...
alter table public.service_pricing_rules
  ...
  add column if not exists flat_price integer,
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'custom_services'
      and column_name = 'fixed_price'
  ) then
    execute 'update public.custom_services set base_price = coalesce(nullif(base_price, 0), fixed_price, 0)';
  end if;
  ...
  if exists (
    ...
    execute 'update public.custom_services set estimated_min_price = coalesce(estimated_min_price, estimated_price), estimated_max_price = coalesce(estimated_max_price, estimated_price)';
  end if;
  ...
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'custom_services'
      and column_name = 'requires_file'
  ) then
    execute 'update public.custom_services set requires_upload = requires_upload or requires_file';
  end if;
  ...
  if exists (
    ...
    execute 'update public.custom_services set is_stackable = stackable';
  end if;

  if exists (
    select 1 from information_schema.columns
    where table_schema = 'public'
      and table_name = 'service_pricing_rules'
      and column_name = 'fixed_price'
  ) then
    execute 'update public.service_pricing_rules set flat_price = coalesce(flat_price, fixed_price)';
  end if;
  ...
  if exists (
    ...
    execute 'update public.service_pricing_rules set unit_price = coalesce(unit_price, estimated_price)';
  end if;
  ...
  if exists (
    ...
    execute 'update public.service_pricing_rules set quote_required = quote_required or requires_review';
  end if;
end $$;
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'service_pricing_rules_no_overlap_active'
  ) then
    alter table public.service_pricing_rules
...
create trigger set_product_minimum_rules_updated_at
before update on public.product_minimum_rules
for each row execute function public.set_updated_at();

drop trigger if exists set_custom_services_updated_at on public.custom_services;
create trigger set_custom_services_updated_at
...
create trigger set_customer_uploads_updated_at
before update on public.customer_uploads
for each row execute function public.set_updated_at();

drop trigger if exists set_saved_configurations_updated_at on public.saved_configurations;
create trigger set_saved_configurations_updated_at
...
create trigger set_configuration_item_services_updated_at
before update on public.configuration_item_services
for each row execute function public.set_updated_at();

drop trigger if exists set_quotation_drafts_updated_at on public.quotation_drafts;
create trigger set_quotation_drafts_updated_at
before update on public.quotation_drafts
for each row execute function public.set_updated_at();
...
create policy "Public can read active minimum rules"
on public.product_minimum_rules for select to anon, authenticated
using (status = 'active');

drop policy if exists "Public can read active custom services" on public.custom_services;
create policy "Public can read active custom services"
...
create policy "Public can read active service pricing rules"
on public.service_pricing_rules for select to anon, authenticated
using (
  status = 'active'
  and exists (
    select 1 from public.custom_services service
    where service.id = service_pricing_rules.service_id
      and service.status = 'active'
  )
);

drop policy if exists "Staff can manage price tiers" on public.product_price_tiers;
create policy "Staff can manage price tiers"
...
create policy "Staff can manage minimum rules"
on public.product_minimum_rules for all to authenticated
using (public.has_staff_role(array['owner','super_admin','sales_admin']))
with check (public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Staff can manage custom services" on public.custom_services;
create policy "Staff can manage custom services"
on public.custom_services for all to authenticated
using (public.has_staff_role(array['owner','super_admin','sales_admin']))
with check (public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Staff can manage service pricing rules" on public.service_pricing_rules;
create policy "Staff can manage service pricing rules"
on public.service_pricing_rules for all to authenticated
using (public.has_staff_role(array['owner','super_admin','sales_admin']))
with check (public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Owners can read customer uploads" on public.customer_uploads;
create policy "Owners can read customer uploads"
on public.customer_uploads for select to authenticated
using (owner_id = auth.uid() or public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Owners can manage customer uploads" on public.customer_uploads;
create policy "Owners can manage customer uploads"
on public.customer_uploads for all to authenticated
using (owner_id = auth.uid() or public.has_staff_role(array['owner','super_admin','sales_admin']))
with check (owner_id = auth.uid() or public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Owners can manage saved configurations" on public.saved_configurations;
create policy "Owners can manage saved configurations"
...
create policy "Public can read shareable configurations"
on public.saved_configurations for select to anon, authenticated
using (
  is_shareable = true
...
create policy "Owners can manage configuration services"
on public.configuration_item_services for all to authenticated
using (
  public.has_staff_role(array['owner','super_admin','sales_admin'])
  or exists (
    select 1 from public.saved_configurations config
    where config.id = configuration_item_services.saved_configuration_id
      and config.owner_id = auth.uid()
  )
)
with check (
  public.has_staff_role(array['owner','super_admin','sales_admin'])
...
create policy "Owners can manage quotation drafts"
on public.quotation_drafts for all to authenticated
using (owner_id = auth.uid() or public.has_staff_role(array['owner','super_admin','sales_admin']))
with check (owner_id = auth.uid() or public.has_staff_role(array['owner','super_admin','sales_admin']));

drop policy if exists "Staff can read quotation items" on public.quotation_draft_items;
create policy "Staff can read quotation items"
on public.quotation_draft_items for select to authenticated
using (
  public.has_staff_role(array['owner','super_admin','sales_admin'])
  or exists (
    select 1 from public.quotation_drafts quotation
    where quotation.id = quotation_draft_items.quotation_draft_id
      and quotation.owner_id = auth.uid()
  )
);

drop policy if exists "Staff can manage quotation items" on public.quotation_draft_items;
create policy "Staff can manage quotation items"
on public.quotation_draft_items for all to authenticated
using (
  public.has_staff_role(array['owner','super_admin','sales_admin'])
  or exists (
    select 1 from public.quotation_drafts quotation
...
create policy "Owners can read own customer designs"
on storage.objects for select to authenticated
using (
  bucket_id = 'customer-designs'
  and (
    owner = auth.uid()
    or public.has_staff_role(array['owner','super_admin','sales_admin'])
  )
);

drop policy if exists "Owners can upload own customer designs" on storage.objects;
create policy "Owners can upload own customer designs"
...
create policy "Owners can delete own customer designs"
...
grant select on public.product_minimum_rules to anon, authenticated;
grant select on public.custom_services to anon, authenticated;
grant select on public.service_pricing_rules to anon, authenticated;
grant insert, update, delete on public.product_price_tiers to authenticated;
grant insert, update, delete on public.product_minimum_rules to authenticated;
grant insert, update, delete on public.custom_services to authenticated;
grant insert, update, delete on public.service_pricing_rules to authenticated;
grant select, insert, update, delete on public.customer_uploads to authenticated;
grant select, insert, update, delete on public.saved_configurations to authenticated;
grant select, insert, update, delete on public.configuration_item_services to authenticated;
grant select, insert, update, delete on public.quotation_drafts to authenticated;
grant select, insert, update, delete on public.quotation_draft_items to authenticated;
