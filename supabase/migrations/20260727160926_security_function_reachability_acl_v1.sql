...
begin
  ...
  if pg_catalog.has_function_privilege('public', next_order_oid, 'EXECUTE')
     ...
     or not pg_catalog.has_function_privilege('authenticated', next_order_oid, 'EXECUTE')
  ...
  if pg_catalog.has_function_privilege('public', next_payment_oid, 'EXECUTE')
     ...
     or not pg_catalog.has_function_privilege('authenticated', next_payment_oid, 'EXECUTE')
  ...
  if pg_catalog.has_function_privilege('public', next_quotation_oid, 'EXECUTE')
     ...
     or not pg_catalog.has_function_privilege('authenticated', next_quotation_oid, 'EXECUTE')
  ...
  loop
    ...
    if function_oid is null then
      raise exception 'SECURITY ACL V1: expected admin function is missing: %', function_row.signature;
    end if;
    ...
    if not pg_catalog.has_function_privilege('anon', function_oid, 'EXECUTE')
      ...
      raise exception 'SECURITY ACL V1: unexpected admin ACL baseline: %', function_row.signature;
    end if;
  end loop;
