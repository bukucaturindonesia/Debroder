...
create or replace function public.update_profile_role(p_profile_id uuid,p_role text)
...
begin
  if not public.has_permission('access_control.manage') then raise exception 'Tidak berwenang mengubah role pengguna'; end if;
  actor_role_value:=public.current_actor_role();
  if actor_role_value not in ('superadmin','super_admin') then raise exception 'Hanya Super Admin yang dapat mengubah role pengguna'; end if;
  if p_role not in ('owner','superadmin','super_admin','admin','sales_admin','designer','production_admin','operator','finance','quality_control','store_staff') then raise exception 'Role tidak valid'; end if;
  ...
  if p_profile_id=auth.uid() and p_role not in ('superadmin','super_admin') then raise exception 'Super Admin tidak dapat menurunkan role akun sendiri'; end if;
  if target_row.role in ('superadmin','super_admin') and p_role not in ('superadmin','super_admin') then
    select count(*) into super_admin_count from public.profiles where role in ('superadmin','super_admin');
    if super_admin_count<=1 then raise exception 'Role Super Admin terakhir tidak boleh diturunkan'; end if;
  end if;
  update public.profiles set role=p_role,updated_at=now() where id=p_profile_id returning * into result_row;
  return result_row;
end
$$;
...
set search_path = ''
as $$
declare old_json jsonb; new_json jsonb; action_value text; role_value text; permission_value text;
begin
  if tg_op='INSERT' then new_json:=to_jsonb(new); action_value:='created'; role_value:=new.role; permission_value:=new.permission_key;
  elsif tg_op='UPDATE' then old_json:=to_jsonb(old); new_json:=to_jsonb(new); action_value:='updated'; role_value:=new.role; permission_value:=new.permission_key;
  else old_json:=to_jsonb(old); action_value:='deleted'; role_value:=old.role; permission_value:=old.permission_key; end if;
  insert into public.system_audit_log(entity_type,entity_id,action,old_value,new_value,actor_id,actor_role,source,metadata)
  values('role_permissions',null,action_value,old_json,new_json,auth.uid(),public.current_actor_role(),'trigger',jsonb_build_object('role',role_value,'permission_key',permission_value));
...
begin
  if not exists (select 1 from pg_trigger where tgrelid='public.profiles'::regclass and tgname='audit_profiles_role_changes' and not tgisinternal) then
    create trigger audit_profiles_role_changes after update of role on public.profiles for each row when (old.role is distinct from new.role) execute function public.audit_row_change();
  end if;
  if not exists (select 1 from pg_trigger where tgrelid='public.role_permissions'::regclass and tgname='audit_role_permissions_changes' and not tgisinternal) then
    create trigger audit_role_permissions_changes after insert or update or delete on public.role_permissions for each row execute function public.audit_role_permission_change();
  end if;
end
$$;

do $$
declare target_table text; trigger_name text;
begin
  foreach target_table in array array['quotation_items','quotation_item_services','order_items','order_item_services','mockup_files','mockup_parts','qc_files','fulfillment_files'] loop
    trigger_name:='audit_'||target_table||'_changes';
    if not exists (select 1 from pg_trigger where tgrelid=('public.'||target_table)::regclass and tgname=trigger_name and not tgisinternal) then
      execute format('create trigger %I after insert or update or delete on public.%I for each row execute function public.audit_row_change()',trigger_name,target_table);
    end if;
  end loop;
end
$$;
...
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='permission_definitions' and policyname='phase13 access readers permission definitions') then
    create policy "phase13 access readers permission definitions" on public.permission_definitions for select to authenticated using (public.has_permission('access_control.read'));
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='role_permissions' and policyname='phase13 access readers role permissions') then
    create policy "phase13 access readers role permissions" on public.role_permissions for select to authenticated using (public.has_permission('access_control.read'));
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='profiles' and policyname='phase13 operational directory read') then
    create policy "phase13 operational directory read" on public.profiles for select to authenticated using (public.has_permission('access_control.read') or public.has_permission('work_item.assign') or public.has_permission('qc.inspect') or public.has_permission('shipping.update') or public.has_permission('mockup.read') or public.has_permission('payment.read'));
  end if;
end
$$;
...
begin
  foreach target_table in array array['orders','order_items','order_item_services','order_status_history'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission order read') then execute format('create policy %I on public.%I for select to authenticated using (public.has_permission(''order.read''))','phase13 permission order read',target_table); end if;
  end loop;
  foreach target_table in array array['quotations','quotation_items','quotation_item_services','quotation_versions','quotation_status_history'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission quotation read') then execute format('create policy %I on public.%I for select to authenticated using (public.has_permission(''quotation.read''))','phase13 permission quotation read',target_table); end if;
  end loop;
  foreach target_table in array array['mockup_sets','mockup_parts','mockup_files','mockup_review_links','mockup_approval_history'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission mockup read') then execute format('create policy %I on public.%I for select to authenticated using (public.has_permission(''mockup.read''))','phase13 permission mockup read',target_table); end if;
  end loop;
  foreach target_table in array array['order_payments','payment_adjustments','payment_submission_links','payment_activity_history'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission payment read') then execute format('create policy %I on public.%I for select to authenticated using (public.has_permission(''payment.read''))','phase13 permission payment read',target_table); end if;
  end loop;
  foreach target_table in array array['fulfillments','fulfillment_items','fulfillment_files','fulfillment_revisions','fulfillment_status_history','fulfillment_deletion_audit'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission shipping view') then execute format('create policy %I on public.%I for select to authenticated using (public.has_permission(''shipping.view''))','phase13 permission shipping view',target_table); end if;
  end loop;
end
$$;
...
begin
  foreach target_table in array array['mockup_sets','mockup_parts','mockup_files','mockup_review_links'] loop
    if not exists (select 1 from pg_policies where schemaname='public' and tablename=target_table and policyname='phase13 permission mockup write') then
      execute format('create policy %I on public.%I for all to authenticated using (public.has_permission(''mockup.write'')) with check (public.has_permission(''mockup.write''))','phase13 permission mockup write',target_table);
    end if;
  end loop;
end
$$;
...
begin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='work_items' and policyname='phase13 production work item read') then
    create policy "phase13 production work item read" on public.work_items for select to authenticated using (public.has_permission('production.view') and (public.current_actor_role()<>'operator' or assigned_to=auth.uid()));
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='job_orders' and policyname='phase13 production job order read') then
    create policy "phase13 production job order read" on public.job_orders for select to authenticated using (public.has_permission('production.view') and (public.current_actor_role()<>'operator' or exists(select 1 from public.work_items work_item where work_item.job_order_id=job_orders.id and work_item.assigned_to=auth.uid())));
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='work_item_status_history' and policyname='phase13 production work history read') then
    create policy "phase13 production work history read" on public.work_item_status_history for select to authenticated using (public.has_permission('production.view') and exists(select 1 from public.work_items work_item where work_item.id=work_item_status_history.work_item_id and (public.current_actor_role()<>'operator' or work_item.assigned_to=auth.uid())));
  end if;
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='job_order_status_history' and policyname='phase13 production job history read') then
    create policy "phase13 production job history read" on public.job_order_status_history for select to authenticated using (public.has_permission('production.view') and exists(select 1 from public.work_items work_item where work_item.job_order_id=job_order_status_history.job_order_id and (public.current_actor_role()<>'operator' or work_item.assigned_to=auth.uid())));
  end if;
end
$$;
...
begin
  if public.current_actor_role()='operator' and old.assigned_to is distinct from auth.uid() then raise exception 'Operator hanya dapat memperbarui Work Item yang ditugaskan kepadanya'; end if;
  return new;
end
$$;
...
begin
  if not exists (select 1 from pg_trigger where tgrelid='public.work_items'::regclass and tgname='guard_operator_work_item_update' and not tgisinternal) then
    create trigger guard_operator_work_item_update before update on public.work_items for each row execute function public.guard_operator_work_item_update();
  end if;
end
$$;
...
returns uuid[] language sql stable security definer set search_path = '' as $$
  select coalesce(array_agg(profile_row.id),array[]::uuid[]) from public.profiles profile_row where profile_row.role in ('owner','superadmin','super_admin','sales_admin','admin','designer','production_admin','operator','finance','quality_control','store_staff')
$$;
create or replace function public.staff_notification_recipients(p_roles text[])
returns uuid[] language sql stable security definer set search_path = '' as $$
  select coalesce(array_agg(profile_row.id),array[]::uuid[]) from public.profiles profile_row where profile_row.role=any(p_roles)
$$;
