alter table public.quotation_item_services
  add column if not exists archived_at timestamptz,
  add column if not exists archived_by uuid,
  add column if not exists archive_reason text;

create index if not exists quotation_item_services_active_idx
  on public.quotation_item_services (quotation_item_id, sort_order, created_at)
  where archived_at is null;

create index if not exists quotation_item_services_archive_idx
  on public.quotation_item_services (quotation_item_id, archived_at desc)
  where archived_at is not null;

create or replace function public.archive_quotation_item_service(
  p_service_id uuid,
  p_reason text default null
)
returns public.quotation_item_services
language plpgsql
security definer
set search_path = public
as $$
declare
  result_row public.quotation_item_services;
  parent_quotation_id uuid;
  parent_status text;
begin
  if not public.has_staff_role(array['owner','superadmin','super_admin','sales_admin','admin']) then
    raise exception 'Not authorized to archive quotation service';
  end if;

  select q.id, q.status
  into parent_quotation_id, parent_status
  ...
  where qis.id = p_service_id;

  if parent_quotation_id is null then
    raise exception 'Quotation service not found';
  end if;
  if parent_status <> 'draft' then
    raise exception 'Only draft quotation services can be archived';
  end if;

  update public.quotation_item_services
  set archived_at = now(),
  ...
  if not found then
    raise exception 'Quotation service is already archived or unavailable';
  end if;

  perform public.refresh_quotation_totals(parent_quotation_id);
  return result_row;
end;
$$;

create or replace function public.restore_quotation_item_service(p_service_id uuid)
returns public.quotation_item_services
language plpgsql
security definer
set search_path = public
...
begin
  if not public.has_staff_role(array['owner','superadmin','super_admin','sales_admin','admin']) then
    raise exception 'Not authorized to restore quotation service';
  end if;
  ...
  if parent_status <> 'draft' then
    raise exception 'Only draft quotation services can be restored';
  end if;
  ...
  if not found then
    raise exception 'Quotation service is not archived';
  end if;
...
create or replace function public.permanently_delete_quotation_item_service(p_service_id uuid)
returns uuid
language plpgsql
security definer
set search_path = public
...
begin
  if not public.has_staff_role(array['owner','superadmin','super_admin']) then
    raise exception 'Only Super Admin can permanently delete quotation services';
  end if;

  select qi.quotation_id
  into parent_quotation_id
  from public.quotation_item_services qis
  join public.quotation_items qi on qi.id = qis.quotation_item_id
  where qis.id = p_service_id
    and qis.archived_at is not null;

  if parent_quotation_id is null then
    raise exception 'Archived quotation service not found';
  end if;

  delete from public.quotation_item_services
  where id = p_service_id
    and archived_at is not null
