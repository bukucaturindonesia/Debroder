-- DEBRODER Migration C1 post-check
-- READ-ONLY. Run only after owner applies C1.
...
begin
  ...
  if pgcrypto_schema is null then
    raise exception 'POST-CHECK FAIL: pgcrypto extension is not installed';
  end if;
  ...
  if create_order_oid is null or submit_proof_oid is null then
    raise exception 'POST-CHECK FAIL: exact function signature missing';
  end if;
  ...
  then
    raise exception 'POST-CHECK FAIL: create_public_order body changed';
  end if;

  if submit_proof_hash <>
    'aecdbdcf357d7fbf8e835e404a13045f231eccdb4faf00059f6aa2a63ba2c1ca'
  then
    raise exception 'POST-CHECK FAIL: submit_public_payment_proof body changed';
  end if;
end
$hash_check$;
...
select count(*) as anonymous_insert_policy_count
...
-- Expected: both superadmin policies remain.
...
-- Canonical transaction RPCs must remain service-role-only.
select
  p.proname,
  pg_catalog.pg_get_function_identity_arguments(p.oid) as identity_arguments,
  pg_catalog.has_function_privilege('anon', p.oid, 'EXECUTE') as anon_execute,
...
where n.nspname = 'public'
  and p.proname in (
    ...
    'submit_customer_order_payment_v2'
  )
